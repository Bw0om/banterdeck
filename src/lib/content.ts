...
export function gamesFor(lang: Lang = 'no'): any[] {
  return games.filter((x) => visible(x, lang) && visible(x.group, lang));
}
/** Egen side for en lek på dette språket, hvis den har det. href kan være felles eller per språk. */
export function hrefFor(x: any, lang: Lang = 'no'): string {
  if (!x || !x.href) return '';
  return typeof x.href === 'string' ? x.href : (x.href[lang] || '');
}
/** Lenken til en lek: egen side hvis den har det, ellers den vanlige regelsiden. */
export function gameHref(x: any, pathFn: (p: string) => string, lang: Lang = 'no'): string {
  return hrefFor(x, lang) || pathFn(`/drinking-games/${x.slug}`);
}
...
/* ---------------- Ordboka delt i to: Ord og Uttrykk ---------------- */
export const DICT_VIEWS: Record<string, { grupper: string[]; sti: string }> = {
  ord: { grupper: ['ord', 'gamle'], sti: '/ord' },
  uttrykk: { grupper: ['uttrykk', 'ordtak'], sti: '/uttrykk' },
};
/** Gruppene som hører til én av de to visningene. */
export function dictGroups(view: string, lang: Lang = 'no'): any[] {
  const v = DICT_VIEWS[view];
  return langGroups(dictSection, lang).filter((g: any) => v.grupper.includes(g.id));
}
export function dictCount(view: string, lang: Lang = 'no'): number {
  return dictGroups(view, lang).reduce((n, g) => n + itemsOf(g, lang).length, 0);
}
/** Hvilken av sidene en oppføring hører hjemme på. */
export function dictViewOf(groupId: string): string {
  return DICT_VIEWS.uttrykk.grupper.includes(groupId) ? 'uttrykk' : 'ord';
}
